...
object MillScalacOptionsOverride {
  object MillCommunityBuild {
    ...
    implicit class MillCommunityBuildScalacOptionsTargetOps(asTarget: mill.T[Seq[String]]) { def mapScalacOptions(scalaVersion: String): mill.T[Seq[String]] = ??? }
  }
  ...
  import mill._
  object module { val scalacOptions = Nil.mapScalacOptions(scalaVersion) }
  object module2 { def scalacOptions: Seq[String] = Seq("-Xprint:typer").mapScalacOptions(scalaVersion) }
  object module3 { def scalacOptions = mill.T(module2.scalacOptions).mapScalacOptions(scalaVersion) }
  class moduleDef {

...
object MillScalacOptionsCommonObject {
  ...
  object Common {
    ...
    val scalacOptions = Seq.empty[String].mapScalacOptions(scalaVersion)
  }
  trait CommonModule extends ScalaModule with MillCommunityBuild.CommunityBuildCoursierModule with MillCommunityBuild.CommunityBuildScalaWorkerPathRefFix {
    override def scalaVersion = mill.Task("3.9.0-RC1")
    override def scalacOptions = Task(Common.scalacOptions)
  }
  object test extends ScalaModule with MillCommunityBuild.CommunityBuildCoursierModule with MillCommunityBuild.CommunityBuildScalaWorkerPathRefFix {
    override def scalaVersion = mill.Task("3.9.0-RC1")
    override def scalacOptions = Task(Common.scalacOptions)
  }
}
